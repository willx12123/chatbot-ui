...
interface Props {
  ...
  onRegenerate: () => void;
  handleSingleMessageModeChange: (val: boolean) => void;
...
export const ChatInput = ({
  ...
  const [
    describeSignalMessageModeVisible,
    setDescribeSignalMessageModeVisible,
  ] = useState(false);
  ...
  function describeSingleMessageMode(
    ...
    e.stopPropagation();
    e.preventDefault();
    setDescribeSignalMessageModeVisible(true);
  }
  ...
  return (
    <>
      {describeSignalMessageModeVisible && (
        <div
          className="fixed top-0 left-0 right-0 bottom-0 z-50 bg-black bg-opacity-40 flex items-center justify-center"
          onClick={() => setDescribeSignalMessageModeVisible(false)}
        >
          <div
            className="bg-gray-800 rounded-xl py-5 px-8 border text-white md:w-[500px]"
            style={{ maxWidth: '80vw' }}
          ...
          >
            ...
            <p>
              默认情况下，将会把当前聊天中所有的聊天记录发送，以获得更连贯的逻辑。但如果你希望忽略掉历史记录，则可以开启「单消息模式」。
            </p>
          </div>
        </div>
      )}
